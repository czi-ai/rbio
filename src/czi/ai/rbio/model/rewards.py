...
def vcm_reward_func(
    gene_perturbed, gene_monitored, completion, task, gene2ensembl_id, model, gene_vocab
):
    """
    ...
    if task == "gene_similarity":
        ...
        answer = [
            x.split("<answer>")[1].split("</answer>")[0].strip() for x in answer_matches
        ]
        if len(answer) == 0:
            return -2.0  # functional mismatch
        p_works_vcm = (
            call_vcm(
                gene_perturbed, gene_monitored, gene2ensembl_id, model, gene_vocab, task
            )
            .detach()
            .numpy()
        )
