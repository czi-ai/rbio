...
def create_marker_genes_df(cell_types, cell_type2gene_name, cell_type2interactions, label):
    """
    ...
    dataset_df['marker_genes'] = dataset_df['cell_type'].apply(lambda x: cell_type2gene_name[x])
    dataset_df['gene_monitored'] = dataset_df['cell_type'].apply(lambda x: [y.strip() for y in cell_type2interactions[x]])
...
def main(
    ...
    ensembl_id2gene_name = pkl.load(open(f'{TF_ROOT_DIR}ensembl_id2gene_name.pkl', 'rb'))
    cell_type2gene_name = {ct : [ensembl_id2gene_name[ensembl_id] if ensembl_id in ensembl_id2gene_name else ensembl_id for ensembl_id in ct_info['gene_ids']] for ct, ct_info in cell_type2ensembl_id.items()}
    ...
    for cell_type_idx, cell_type in enumerate(filtered_cell_types):
        ...
        sig_cell_type_pmis = sorted_tfs[np.argwhere(cell_type_pmis > pmi_cutoff)].squeeze()
        not_sig_type_pmis = sorted_tfs[np.argwhere(cell_type_pmis <= pmi_cutoff)].squeeze()
        ...
        if len(not_sig_type_pmis) > min_not_sig_interactions_to_include:
            not_sig_type_pmis = np.random.choice(not_sig_type_pmis, min_not_sig_interactions_to_include, replace = False)
    ...
    df_sig = create_marker_genes_df(filtered_cell_types, cell_type2gene_name, cell_type2significant_TFs, 'yes')
    
    # Not Significant TFs interactions based on Marker Genes
    df_not_sig = create_marker_genes_df(filtered_cell_types, cell_type2gene_name, cell_type2less_significant_TFs, 'no')
    ...
    dataset_df['user_prompt'] = dataset_df.apply(lambda x: f"In a cell that has marker genes {', '.join(x['marker_genes'])} expressed, is transcription factor {x['gene_monitored']} likely to be activated? The answer is either yes or no.", 1)
    ...
    if binary_class_confidences:
        ...
        output_filepath = f'{output_dir}/TF_MGs2TFs_pmi_{pmi_cutoff}-train-v0.0.3_binary.csv'
    else:
        dataset_df['class_confidences'] = dataset_df.apply(lambda x: compute_soft_class_confidences(x, cell_type2tf_pmi, ['cell_type', 'gene_monitored']) , 1)
        output_filepath = f'{output_dir}/TF_MGs2TFs_pmi_{pmi_cutoff}-train-v0.0.3_soft.csv'
