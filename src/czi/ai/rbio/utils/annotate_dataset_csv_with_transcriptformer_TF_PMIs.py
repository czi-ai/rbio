...
def top_k_indices(array, k, reverse=True):
    """
    Returns indices of topk scores in a given array of precomputed interaction values

    Args:
        array: given array containing confidence scors
        k: topk scores to return
        reverse: if False, return least topk scores
    Returns:
        indices of topk scores
    """
...
def create_pmis_df(gene_indicesA, gene_indicesB, label):
    """
    Creates dataframe based on gene pairs and a given label;
    Annotates dataframe with a prompt.

    Args:
        gene_indicesA: indices of a list of genes
        gene_indicesB: indices of another list of genes
        label: 'yes' if gene_indicesA and gene_indicesB have a significant interaction, 'no' otherwise
...
def main(
    ...
    least_significant_gene_pairs = {
        (idx2gene[gene_A_idx], idx2gene[gene_B_idx]): pmis[gene_A_idx, gene_B_idx]
        for gene_A_idx, gene_B_idx in zip(
            topk_gene_indices1_last, topk_gene_indices2_last
        )
    }
    ...
    dataset_df_least_sig = create_pmis_df(
        gene_indicesA_least, gene_indicesB_least, label="no"
    )
