...
def create_transcription_factors_df(transcription_factors, tf2interactions, label):
    ...
    dataset_sig_df['gene_monitored'] = dataset_sig_df['transcription_factor'].apply(lambda x: tf2interactions[x])
...
def main(
    ...
    print(f'There are {len(tfs)} transcription factors and a total of {len(gene_vocab)} total genes in the vocab')
    ...
    for tf_idx, tf in enumerate(tfs):
        ...
        tf2sig_interactions[tf] = [idx2gene[x[0]] for x in np.argwhere(tf_p_values <= p_value)]

        not_sig_interactions_indices = np.argwhere(tf_p_values > p_value).reshape(-1, )
        num_samples = min(len(not_sig_interactions_indices), min_not_sig_interactions_to_include)
        not_sig_interactions_indices_sampled = np.random.choice(not_sig_interactions_indices, num_samples, replace = False)
        tf2not_sig_interactions[tf] = [idx2gene[x] for x in np.random.choice(not_sig_interactions_indices, num_samples, replace = False)]
       
    # Significant interactions
    dataset_sig_df = create_transcription_factors_df(tfs, tf2sig_interactions, 'yes')
    
    # Not Significant interactions
    dataset_not_sig_df = create_transcription_factors_df(tfs,  tf2not_sig_interactions, 'no')
    
    # Combine significan and non-significant interactions
    dataset_df = pd.concat([dataset_sig_df, dataset_not_sig_df])
    dataset_df['user_prompt'] = dataset_df.apply(lambda x: f"If transcription factor {x['transcription_factor']} is activated, is expression of gene {x['gene_monitored']} going to be high? The answer is either yes or no.", 1)
    dataset_df['classes'] = 'yes|no'
    dataset_df['keywords'] = dataset_df.apply(lambda x: '|'.join([x['transcription_factor'], x['gene_monitored']]), 1)
    ...
    if binary_class_confidences:
        ...
        output_filepath = f'{output_dir}/TF_TFs2genes_p_{p_value}-train-v0.0.3_binary.csv'
    else:
        dataset_df['class_confidences'] = dataset_df.apply(lambda x: compute_soft_class_confidences(x, tf_pairs2p_vals, ['transcription_factor', 'gene_monitored']) , 1)
