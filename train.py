...
TRAIN_DATASET_PATH = (
    "/mnt/czi-sci-ai/project-rbio/AutoSync/Datasets/PertQA-DE/k562-train-latest.csv"
)
TEST_DATASET_PATH = (
    "/mnt/czi-sci-ai/project-rbio/AutoSync/Datasets/PertQA-DE/k562-test-latest.csv"
)
...
train_dataset = Dataset.from_generator(
    dataset_gen, gen_kwargs={"dataset": train_df, "tokenizer": tokenizer}
)
test_dataset = Dataset.from_generator(
    dataset_gen, gen_kwargs={"dataset": test_df, "tokenizer": tokenizer}
)
...
def reward(completions, label, gene_perturbed, gene_monitored, **kwargs):
    ...
    for completion, lbl, gp, gm in zip(
        ...
        vcm_reward_gene_similarity = vcm_reward_func(
            gp, gm, completion, "gene_similarity", GENE2ENSEMBL_ID, TF_MODEL, GENE_VOCAB
        )
        ...
        total_score = (
            format_reward + answer_reward + mention_reward + vcm_reward_gene_similarity
        )
        print(
            format_reward,
            answer_reward,
            mention_reward,
            vcm_reward_gene_similarity,
            total_score,
        )
    ...
    if count % 10 == 0:
        ...
        mlflow.log_metric(
            "vcm_reward_gene_similarity", vcm_reward_gene_similarity, step=count
        )
...
def train_fn(
    ...
    pipe = pipeline(
        "text-generation", model=trainer.model, tokenizer=tokenizer, max_length=1024
    )
...
@click.option(
    "--train-dataset-path",
    help="Train Dataset CSV file path",
    required=True,
    default="/mnt/czi-sci-ai/project-rbio/AutoSync/Datasets/PertQA-DE/k562-train-latest.csv",
)
@click.option(
    "--test-dataset-path",
    help="Test Dataset CSV file path",
    required=True,
    default="/mnt/czi-sci-ai/project-rbio/AutoSync/Datasets/PertQA-DE/k562-test-latest.csv",
)
@click.option(
    "--model-name",
