...
import sys
module_dir = os.path.join(os.path.dirname(__file__), "../transcriptformer/src", "transcriptformer")
...
def call_vcm(gene_perturbed, gene_monitored, gene2ensembl_id, model, gene_vocab, verification_type = 'gene_similarity'):
    gene_perturbed_ensembl_id = str(np.random.choice(gene2ensembl_id[gene_perturbed])) if gene_perturbed in gene2ensembl_id else '[PAD]'
    gene_monitored_ensembl_id = str(np.random.choice(gene2ensembl_id[gene_monitored])) if gene_monitored in gene2ensembl_id else '[PAD]' 
    gene_perturbed_index = gene_vocab[gene_perturbed_ensembl_id] if gene_perturbed_ensembl_id in gene_vocab else gene_vocab['[PAD]']
    gene_monitored_index = gene_vocab[gene_monitored_ensembl_id] if gene_monitored_ensembl_id in gene_vocab else gene_vocab['[PAD]']
